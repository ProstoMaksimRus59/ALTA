...
def clear(mode): #Ну даже не знаю??? что это делает??? :D
    ...
    if mode != "0": #Если не авто чистка, то показать версию.
        print("Версия ALTA v4.1_3 by Prosto_Maksim")
print("Загрузка.    1/23")
...
def lvlcal(fps,Timings,seting):
    ...
    if seting != "2":
        print("\nВерсия ALTA v4.1_3 by Prosto_Maksim")
...
def settingfiles(mode, typE, Number): #Отвечает за сохранения настроек в файл. ГОВНО КОД!
